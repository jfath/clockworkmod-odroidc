...
#get display mode from arg, or property, or default
if [ -n "$1" ]
then
    mode="$1"
else
    mode=$(getprop display.mode)
    if [ -z "$mode" ]
    then
        mode="720p"
    fi
...
#overscan settings
left=$(getprop display.left)
if [ -z "$left" ]
then
    left=0
fi

right=$(getprop display.right)
if [ -z "$right" ]
then
    right=0
fi

top=$(getprop display.top)
if [ -z "$top" ]
then
    top=0
fi

bottom=$(getprop display.bottom)
if [ -z "$bottom" ]
then
    bottom=0
fi
